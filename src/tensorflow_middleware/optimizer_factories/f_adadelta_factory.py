...
def call(self, operation: dict) -> None:
    self.project_data[operation["uid"]] = keras.optimizers.Adadelta(
        learning_rate=(
            operation["args"]["learning_rate"]
            if "learning_rate" in operation["args"]
            else 0.001
        ),
        rho=(operation["args"]["rho"] if "rho" in operation["args"] else 0.95),
        epsilon=(
        ...
        name=(operation["args"]["name"] if "name" in operation["args"] else "Adadelta"),
    )
