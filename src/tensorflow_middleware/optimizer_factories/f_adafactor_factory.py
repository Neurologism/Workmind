...
def call(self, operation: dict) -> None:
    self.project_data[operation["uid"]] = keras.optimizers.Adafactor(
        ...
        beta_2_decay=(
            operation["args"]["beta_2_decay"]
            if "beta_2_decay" in operation["args"]
            else -0.8
        ),
        epsilon_1=(
            operation["args"]["epsilon_1"]
            if "epsilon_1" in operation["args"]
            else 1e-30
        ),
        epsilon_2=(
            operation["args"]["epsilon_2"]
            if "epsilon_2" in operation["args"]
            else 0.001
        ),
        clip_treshold=(
            operation["args"]["clip_trheshold"]
            if "clip_trheshold" in operation["args"]
            else 1.0
        ),
        relative_step=(
            operation["args"]["relative_step"]
            if "relative_step" in operation["args"]
            else True
        ),
        weight_decay=(
        ...
        name=(
            operation["args"]["name"] if "name" in operation["args"] else "Adafactor"
        ),
    )
