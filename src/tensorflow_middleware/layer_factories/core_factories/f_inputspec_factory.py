...
def call(self, operation: dict) -> None:
    self.project_data[operation["uid"]] = keras.layers.InputSpec(
        ...
        max_ndim=(
            operation["args"]["max_ndim"] if "max_ndim" in operation["args"] else None
        ),
        min_ndim=(
            operation["args"]["min_ndim"] if "min_ndim" in operation["args"] else None
        ),
        axes=(operation["args"]["axes"] if "axes" in operation["args"] else None),
        allow_last_axis_squeeze=(
            operation["args"]["allow_last_axis_squeeze"]
            if "allow_last_axis_squeeze" in operation["args"]
            else None
        ),
