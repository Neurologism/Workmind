...
def call(self, operation: dict) -> None:
    self.project_data[operation["uid"]] = keras.layers.BatchNormalization(
        ...
        momentum=(
            operation["args"]["momentum"] if "momentum" in operation["args"] else 0.99
        ),
        epsilon=(
            operation["args"]["epsilon"] if "epsilon" in operation["args"] else 0.001
        ),
        ...
        moving_mean_initializer=(
            operation["args"]["moving_mean_initializer"]
            if "moving_mean_initializer" in operation["args"]
            else "zeros"
        ),
        moving_variance_initializer=(
            operation["args"]["moving_variance_initializer"]
            if "moving_variance_initializer" in operation["args"]
            else "ones"
        ),
        beta_regularizer=(
        ...
        synchronized=(
            operation["args"]["synchronized"]
            if "synchronized" in operation["args"]
            else False
        ),
