...
def call(self, operation: dict) -> None:
    self.project_data[operation["uid"]] = keras.layers.Embedding(
        ...
        embeddings_regularizer=(self.project_data[operation["args"]["embeddings_regularizer"]] if "embeddings_regularizer" in operation["args"] else None),
        embeddings_constraint=(self.project_data[operation["args"]["embeddings_constraint"]] if "embeddings_constraint" in operation["args"] else None),
        mask_zero=(operation["args"]["mask_zero"] if "mask_zero" in operation["args"] else False),
        weights=(self.project_data[operation["args"]["weights"]] if "weights" in operation["args"] else None),
