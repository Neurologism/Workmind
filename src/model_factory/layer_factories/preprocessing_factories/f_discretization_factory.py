...
def call(self, operation: dict) -> None:
    self.project_data[operation["uid"]] = keras.layers.Discretization(
        bin_boundaries=(operation["args"]["bin_boundaries"] if "bin_boundaries" in operation["args"] else None),
        num_bins=(operation["args"]["num_bins"] if "num_bins" in operation["args"] else None),
        epsilon=(operation["args"]["epsilon"] if "epsilon" in operation["args"] else 0.01),
