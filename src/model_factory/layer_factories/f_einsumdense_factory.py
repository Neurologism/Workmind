...
def call(self, operation: dict) -> None:
    self.project_data[operation["uid"]] = keras.layers.EinsumDense(
        operation["args"]["equation"],
        (operation["args"]["output_shape"] if "output_shape" in operation["args"] else None),
        activation=(operation["args"]["activation"] if "activation" in operation["args"] else None),
        bias_axes=(operation["args"]["bias_axes"] if "bias_axes" in operation["args"] else None),
