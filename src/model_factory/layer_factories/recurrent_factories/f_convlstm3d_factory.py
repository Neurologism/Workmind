...
def call(self, operation: dict) -> None:
    self.project_data[operation["uid"]] = keras.layers.ConvLSTM3D(
        filters=operation["args"]["filters"],
        kernel_size=operation["args"]["kernel_size"],
        strides=(operation["args"]["strides"] if "strides" in operation["args"] else (1, 1, 1)),
        ...
        dilation_rate=(operation["args"]["dilation_rate"] if "dilation_rate" in operation["args"] else (1, 1, 1)),
        ...
        unit_forget_bias=(operation["args"]["unit_forget_bias"] if "unit_forget_bias" in operation["args"] else True),
