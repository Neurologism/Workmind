...
def call(self, operation: dict) -> None:
    self.project_data[operation["uid"]] = keras.optimizers.AdamW(
        learning_rate=(operation["args"]["learning_rate"] if "learning_rate" in operation["args"] else 0.001),
        weight_decay=(operation["args"]["weight_decay"] if "weight_decay" in operation["args"] else 0.004),
        ...
        amsgrad=(operation["args"]["amsgrad"] if "amsgrad" in operation["args"] else False),
        ...
        name=(operation["args"]["name"] if "name" in operation["args"] else "AdamW"),
    )