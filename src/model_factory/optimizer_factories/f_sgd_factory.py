...
def call(self, operation: dict) -> None:
    self.project_data[operation["uid"]] = keras.optimizers.SGD(
        learning_rate=(operation["args"]["learning_rate"] if "learning_rate" in operation["args"] else 0.01),
        momentum=(operation["args"]["momentum"] if "momentum" in operation["args"] else 0.0),
        nesterov=(operation["args"]["nesterov"] if "nesterov" in operation["args"] else False),
