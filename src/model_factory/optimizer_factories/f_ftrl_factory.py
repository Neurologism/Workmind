...
def call(self, operation: dict) -> None:
    self.project_data[operation["uid"]] = keras.optimizers.Ftrl(
        learning_rate=(operation["args"]["learning_rate"] if "learning_rate" in operation["args"] else 0.001),
        learning_rate_power=(operation["args"]["learning_rate_power"] if "learning_rate_power" in operation["args"] else -0.5),
        initial_accumulator_value=(operation["args"]["initial_accumulator_value"] if "initial_accumulator_value" in operation["args"] else 0.1),
        l1_regularization_strength=(operation["args"]["l1_regularization_strength"] if "l1_regularization_strength" in operation["args"] else 0.0),
        l2_regularization_strength=(operation["args"]["l2_regularization_strength"] if "l2_regularization_strength" in operation["args"] else 0.0),
        l2_shrinkage_regularization_strength=(operation["args"]["l2_shrinkage_regularization_strength"] if "l2_shrinkage_regularization_strength" in operation["args"] else 0.0),
        beta=(operation["args"]["beta"] if "beta" in operation["args"] else 0.0),
        ...
        name=(operation["args"]["name"] if "name" in operation["args"] else "Ftrl"),
    )